  ...
  function (
    ...
    $scope.doLogin = function () {
      if (this.loginForm.$valid) {
        ...
        $http(req).then(function loginCallback(loginResponse) {
          if (loginResponse.status === 200) {
            if (loginResponse.data.status === 'success') {
              try {
                localStorageService.set(
                  'token',
                  loginResponse.data.data.attributes.token
                );
              } catch (error) {
              ...
              $http(req).then(function userDataCallback(userDataResponse) {
        ...
        let alerts = '';
        $rootScope.getFormErrors(this.loginForm).forEach(function (msg, idx) {
...
const AdminApp = angular
  .module('AdminApp')
  .controller('LoginController', loginControllerFactory);
